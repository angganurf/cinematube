...
export const meta = mergeMeta<typeof loader, { 'routes/tv-shows+/$tvId': typeof tvIdLoader }>(
  ({ matches, params }) => {
    ...
    const { name } = detail || {};
    return [
      { title: `Cinematube - ${name} - Crew` },
      { property: 'og:title', content: `Cinematube - ${name} - Crew` },
      { property: 'og:url', content: `https://cinematube.vercel.app/tv-shows/${params.tvId}/crew` },
      { property: 'twitter:title', content: `Cinematube - ${name} - Crew` },
    ];
  },
);
