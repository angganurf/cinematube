...
>(({ params, matches }) => {
  ...
  const { name } = detail || {};
  return [
    { title: `Cinematube - ${name} ${seasonDetail?.name || ''} - Crew` },
    {
      property: 'og:url',
      content: `https://cinematube.vercel.app/tv-shows/${params.tvId}/season/${params.seasonId}/crew`,
    },
    { property: 'og:title', content: `Cinematube - ${name} ${seasonDetail?.name || ''} - Crew` },
    { name: 'twitter:title', content: `Cinematube - ${name} ${seasonDetail?.name || ''} - Crew` },
  ];
});
